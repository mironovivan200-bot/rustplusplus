...
module.exports = {
    getEmbed: function (options = {}) {
        ...
        if (options.hasOwnProperty('thumbnail') && options.thumbnail !== '')
    ...
    getStorageMonitorEmbed: function (guildId, serverId, entityId) {
        ...
        if (itemName === '')
            itemName = Client.client.intlGet(guildId, 'empty');
        if (itemQuantity === '')
            itemQuantity = Client.client.intlGet(guildId, 'empty');
        return module.exports.getEmbed({
            title: `${entity.name}${grid}`,
            color: Constants.COLOR_DEFAULT,
            description: description,
    ...
    getSmartSwitchGroupEmbed: function (guildId, serverId, groupId) {
        ...
        Client.client.setInstance(guildId, instance);
        if (switchName === '')
            switchName = Client.client.intlGet(guildId, 'none');
        if (switchId === '')
            switchId = Client.client.intlGet(guildId, 'none');
        if (switchActive === '')
            switchActive = Client.client.intlGet(guildId, 'none');
        return module.exports.getEmbed({
            title: group.name,
            color: Constants.COLOR_DEFAULT,
    ...
    getStorageMonitorRecycleEmbed: function (guildId, serverId, entityId, items) {
        ...
        if (itemQuantity === '')
            itemQuantity = Client.client.intlGet(guildId, 'empty');
        embed.addFields({ name: Client.client.intlGet(guildId, 'item'), value: itemName, inline: true }, { name: Client.client.intlGet(guildId, 'quantity'), value: itemQuantity, inline: true });
        return embed;
    },
    ...
    getUpdateServerInformationEmbed: function (rustplus) {
        ...
        if (time[1] !== null) {
            embed.addFields({ name: timeLeftTitle, value: `\`${time[1]}\``, inline: true }, { name: '\u200B', value: '\u200B', inline: true }, { name: '\u200B', value: '\u200B', inline: true });
        }
        else {
            embed.addFields({ name: '\u200B', value: '\u200B', inline: false });
        }
        embed.addFields({ name: mapSizeFieldName, value: `\`${rustplus.info.mapSize}\``, inline: true }, { name: mapSeedFieldName, value: `\`${rustplus.info.seed}\``, inline: true }, { name: mapSaltFieldName, value: `\`${rustplus.info.salt}\``, inline: true }, { name: mapFieldName, value: `\`${rustplus.info.map}\``, inline: true });
    ...
    getCredentialsShowEmbed: async function (guildId) {
        ...
        if (names === '')
            names = Client.client.intlGet(guildId, 'empty');
        if (steamIds === '')
            steamIds = Client.client.intlGet(guildId, 'empty');
        if (hoster === '')
            hoster = Client.client.intlGet(guildId, 'empty');
        return module.exports.getEmbed({
            color: Constants.COLOR_DEFAULT,
    ...
    getHelpEmbed: function (guildId) {
        ...
        const description = `→ [${Client.client.intlGet(guildId, 'commandsHelpHowToCredentials')}](${credentials})\n` +
