...
module.exports = async (client, guild, category) => {
    ...
    await addTextChannel(client.intlGet(guild.id, 'channelNameStorageMonitors'), 'storageMonitors', client, guild, category);
...
async function addTextChannel(name, idName, client, guild, parent, permissionWrite = false) {
    ...
    if (channel === undefined) {
        ...
        try {
            ...
            client.log(client.intlGet(null, 'errorCap'), client.intlGet(null, 'couldNotSetParent', { channelId: channel.id }), 'error');
        }
    }
