...
class Battlemetrics {
    ...
    #parseProfileDataApiResponse(data) {
        ...
        for (const name of data.included) {
            if (name.type !== 'identifier')
                continue;
            if (!name.hasOwnProperty('attributes'))
                continue;
            if (!name['attributes'].hasOwnProperty('type'))
                continue;
            if (name['attributes']['type'] !== 'name')
                continue;
            if (!name['attributes'].hasOwnProperty('identifier'))
                continue;
            if (!name['attributes'].hasOwnProperty('lastSeen'))
                continue;
            parsed.push({
    ...
    async request(api_call) {
        ...
        if (response.status !== 200) {
            Client.client.log(Client.client.intlGet(null, 'errorCap'), Client.client.intlGet(null, 'battlemetricsApiRequestFailed', { api_call: api_call }), 'error');
            return null;
        }
    ...
    async setup() {
        if (this.id === null && this.name === null) {
            Client.client.log(Client.client.intlGet(null, 'errorCap'), Client.client.intlGet(null, 'battlemetricsIdAndNameMissing'), 'error');
            return;
        }
        if (this.id === null && this.name !== null) {
    ...
    async getServerIdFromName(name) {
        ...
        if (response.status !== 200) {
            Client.client.log(Client.client.intlGet(null, 'errorCap'), Client.client.intlGet(null, 'battlemetricsApiRequestFailed', { api_call: search }), 'error');
            return null;
        }
    ...
    async evaluation(data = null, firstTime = false) {
        ...
        if (!data) {
            ...
            Client.client.log(Client.client.intlGet(null, 'errorCap'), Client.client.intlGet(null, 'battlemetricsFailedToUpdate', { server: this.id }), 'error');
            return false;
        }
        this.lastUpdateSuccessful = true;
        this.data = data;
        ...
        this.#evaluateServerParameter('server_rust_ent_cnt_i', this.server_rust_ent_cnt_i, details.rust_ent_cnt_i, firstTime);
        ...
        this.#evaluateServerParameter('server_rust_headerimage', this.server_rust_headerimage, details.rust_headerimage, firstTime);
        ...
        this.#evaluateServerParameter('server_rust_world_seed', this.server_rust_world_seed, details.rust_world_seed, firstTime);
        this.#evaluateServerParameter('server_rust_world_size', this.server_rust_world_size, details.rust_world_size, firstTime);
        this.#evaluateServerParameter('server_rust_description', this.server_rust_description, details.rust_description, firstTime);
        this.#evaluateServerParameter('server_rust_modded', this.server_rust_modded, details.rust_modded, firstTime);
        this.#evaluateServerParameter('server_rust_queued_players', this.server_rust_queued_players, details.rust_queued_players, firstTime);
        this.#evaluateServerParameter('server_rust_gamemode', this.server_rust_gamemode, details.rust_gamemode, firstTime);
        this.#evaluateServerParameter('server_rust_born', this.server_rust_born, details.rust_born, firstTime);
        this.#evaluateServerParameter('server_rust_last_seed_change', this.server_rust_last_seed_change, details.rust_last_seed_change, firstTime);
        this.#evaluateServerParameter('server_rust_last_wipe', this.server_rust_last_wipe, details.rust_last_wipe, firstTime);
        this.#evaluateServerParameter('server_rust_last_wipe_ent', this.server_rust_last_wipe_ent, details.rust_last_wipe_ent, firstTime);
        this.#evaluateServerParameter('server_serverSteamId', this.server_serverSteamId, details.serverSteamId, firstTime);
        ...
        for (const entity of included) {
            ...
            if (!this.players.hasOwnProperty(entity.id)) { /* New Player */
                ...
                if (firstTime)
                    this.players[entity.id]['firstTime'] = firstTime.value;
                /* Other */
                this.players[entity.id]['url'] = this.GET_BATTLEMETRICS_PLAYER_URL(entity.id);
        ...
        for (const [playerId, content] of Object.entries(this.players)) {
            if (content['status'] === false)
                this.offlinePlayers.push(playerId);
        }
        this.update(data);
        return true;
    }
