...
class Map {
    constructor(map, rustplus) {
        ...
        this._mapMarkerImageMeta = {
            ...
            player: {
                image: Path.join(__dirname, '..', 'resources/images/markers/player.png'), size: 20, type: 1, jimp: null
            },
            shop: {
                image: Path.join(__dirname, '..', 'resources/images/markers/shop.png'), size: 20, type: 3, jimp: null
            },
            chinook: {
                image: Path.join(__dirname, '..', 'resources/images/markers/chinook.png'), size: 50, type: 4, jimp: null
            },
            cargo: {
                image: Path.join(__dirname, '..', 'resources/images/markers/cargo.png'), size: 100, type: 5, jimp: null
            },
            blade: {
                image: Path.join(__dirname, '..', 'resources/images/markers/blade.png'), size: 25, type: 7, jimp: null
            },
            heli: {
                image: Path.join(__dirname, '..', 'resources/images/markers/heli.png'), size: 20, type: 8, jimp: null
            },
            tunnels: {
                image: Path.join(__dirname, '..', 'resources/images/markers/tunnels.png'), size: 35, type: 9, jimp: null
            },
            tunnels_link: {
                image: Path.join(__dirname, '..', 'resources/images/markers/tunnels_link.png'), size: 35, type: 10,
                jimp: null
            }
        };
        this._monumentInfo = {
    ...
    async mapAppendMonuments() {
        if (this.rustplus.info === null) {
            this.rustplus.log(Client.client.intlGet(null, 'warningCap'), Client.client.intlGet(null, 'couldNotAppendMapMonuments'));
            return;
        }
        for (let monument of this.monuments) {
            ...
            try {
                if (monument.token === "train_tunnel_display_name") {
                    ...
                    this.mapMarkerImageMeta.map.jimp.composite(this.mapMarkerImageMeta.tunnels.jimp, x - (size / 2), y - (size / 2));
                }
                else if (monument.token === "train_tunnel_link_display_name") {
                    let size = this.mapMarkerImageMeta.tunnels_link.size;
                    this.mapMarkerImageMeta.map.jimp.composite(this.mapMarkerImageMeta.tunnels_link.jimp, x - (size / 2), y - (size / 2));
                }
                else {
    ...
    async mapAppendMarkers() {
        if (this.rustplus.info === null) {
            this.rustplus.log(Client.client.intlGet(null, 'warningCap'), Client.client.intlGet(null, 'couldNotAppendMapMarkers'));
            return;
        }
        ...
        for (let marker of mapMarkers.mapMarkers.markers) {
            ...
            try {
                ...
                this.mapMarkerImageMeta.map.jimp.composite(this.mapMarkerImageMeta[markerImageMeta].jimp, x - (size / 2), y - (size / 2));
            }
            catch (e) {
                /* Ignore */
            }
    ...
    async writeMap(markers, monuments) {
        ...
        await this.mapMarkerImageMeta.map.jimp.writeAsync(this.mapMarkerImageMeta.map.image.replace('clean.png', 'full.png'));
        try {
            const image = Gm(this.mapMarkerImageMeta.map.image.replace('clean.png', 'full.png'));
            if (this.rustplus.info === null) {
                this.rustplus.log(Client.client.intlGet(null, 'warningCap'), Client.client.intlGet(null, 'couldNotAppendMapTracers'));
                return;
            }
            if (!markers)
                return;
            ...
            this.rustplus.log(Client.client.intlGet(null, 'warningCap'), Client.client.intlGet(null, 'couldNotAddStepTracers'));
        }
    }
