...
class MapMarkers {
        ...
        for (let marker of newMarkers) {
            ...
            if (!this.rustplus.isFirstPoll) {
                if (!this.knownVendingMachines.some(e => e.x === marker.x && e.y === marker.y)) {
                    this.rustplus.sendEvent(this.rustplus.notificationSettings.vendingMachineDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'newVendingMachine', { location: pos.string }), null, Constants.COLOR_NEW_VENDING_MACHINE);
                }
            }
    ...
    updateCH47s(mapMarkers) {
        ...
        for (let marker of newMarkers) {
            ...
            if (!this.rustplus.isFirstPoll) {
                for (let oilRig of smallOilRig) {
                    if (Map.getDistance(marker.x, marker.y, oilRig.x, oilRig.y) <=
                        ...
                        this.rustplus.sendEvent(this.rustplus.notificationSettings.heavyScientistCalledSetting, this.client.intlGet(this.rustplus.guildId, 'heavyScientistsCalledSmall', { location: oilRigLocation.location }), 'small', Constants.COLOR_HEAVY_SCIENTISTS_CALLED_SMALL, this.rustplus.isFirstPoll, 'small_oil_rig_logo.png');
                        if (this.crateSmallOilRigTimer) {
                            this.crateSmallOilRigTimer.stop();
                        }
                        let instance = this.client.getInstance(this.rustplus.guildId);
                        this.crateSmallOilRigTimer = new Timer.timer(this.notifyCrateSmallOilRigOpen.bind(this), instance.serverList[this.rustplus.serverId].oilRigLockedCrateUnlockTimeMs, oilRigLocation.location);
            ...
            if (!found && !this.rustplus.isFirstPoll) {
                for (let oilRig of largeOilRig) {
                    if (Map.getDistance(marker.x, marker.y, oilRig.x, oilRig.y) <=
                        ...
                        this.rustplus.sendEvent(this.rustplus.notificationSettings.heavyScientistCalledSetting, this.client.intlGet(this.rustplus.guildId, 'heavyScientistsCalledLarge', { location: oilRigLocation.location }), 'large', Constants.COLOR_HEAVY_SCIENTISTS_CALLED_LARGE, this.rustplus.isFirstPoll, 'large_oil_rig_logo.png');
                        if (this.crateLargeOilRigTimer) {
                            this.crateLargeOilRigTimer.stop();
                        }
                        let instance = this.client.getInstance(this.rustplus.guildId);
                        this.crateLargeOilRigTimer = new Timer.timer(this.notifyCrateLargeOilRigOpen.bind(this), instance.serverList[this.rustplus.serverId].oilRigLockedCrateUnlockTimeMs, oilRigLocation.location);
            ...
            if (!found) {
                ...
                if (Map.isOutsideGridSystem(marker.x, marker.y, mapSize, offset)) {
                    this.rustplus.sendEvent(this.rustplus.notificationSettings.chinook47DetectedSetting, this.client.intlGet(this.rustplus.guildId, 'chinook47EntersMap', { location: pos.string }), 'chinook', Constants.COLOR_CHINOOK47_ENTERS_MAP);
                }
                else {
                    this.rustplus.sendEvent(this.rustplus.notificationSettings.chinook47DetectedSetting, this.client.intlGet(this.rustplus.guildId, 'chinook47Located', { location: pos.string }), 'chinook', Constants.COLOR_CHINOOK47_LOCATED);
                }
        ...
        for (let marker of leftMarkers) {
            if (marker.ch47Type === 'crate') {
                ...
                this.rustplus.log(this.client.intlGet(null, 'eventCap'), this.client.intlGet(null, 'chinook47LeftMap', { location: marker.location.string }));
            }
    ...
    updateCargoShips(mapMarkers) {
        ...
        for (let marker of newMarkers) {
            ...
            if (Map.isOutsideGridSystem(marker.x, marker.y, mapSize, offset)) {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipEntersMap', { location: pos.string }), 'cargo', Constants.COLOR_CARGO_SHIP_ENTERS_MAP);
                let instance = this.client.getInstance(this.rustplus.guildId);
                this.cargoShipEgressTimers[marker.id] = new Timer.timer(this.notifyCargoShipEgress.bind(this), instance.serverList[this.rustplus.serverId].cargoShipEgressTimeMs, marker.id);
                this.cargoShipEgressTimers[marker.id].start();
            }
            else {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipLocated', { location: pos.string }), 'cargo', Constants.COLOR_CARGO_SHIP_LOCATED);
            }
            this.cargoShips.push(marker);
        }
        /* CargoShip markers that have left. */
        for (let marker of leftMarkers) {
            this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipLeftSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipLeftMap', { location: marker.location.string }), 'cargo', Constants.COLOR_CARGO_SHIP_LEFT_MAP);
                ...
                for (const harbor of harbors) {
                    if (Map.getDistance(marker.x, marker.y, harbor.x, harbor.y) <= Constants.HARBOR_DOCK_DISTANCE) {
                        if (marker.x === cargoShip.x && marker.y === cargoShip.y) {
                            ...
                            this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipDockingAtHarborSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipDockingAtHarbor', { location: harborLocation.location }), 'cargo', Constants.COLOR_CARGO_SHIP_DOCKED);
                        }
                    }
                }
            }
                ...
                for (const harbor of harbors) {
                    if (Map.getDistance(marker.x, marker.y, harbor.x, harbor.y) <= Constants.HARBOR_DOCK_DISTANCE) {
                        if (marker.x !== cargoShip.x || marker.y !== cargoShip.y) {
                            ...
                            this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipDockingAtHarborSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipLeftHarbor', { location: harborLocation.location }), 'cargo', Constants.COLOR_CARGO_SHIP_DOCKED);
                        }
                    }
                }
            }
    ...
    updatePatrolHelicopters(mapMarkers) {
        ...
        for (let marker of newMarkers) {
            ...
            if (Map.isOutsideGridSystem(marker.x, marker.y, mapSize, offset)) {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.patrolHelicopterDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'patrolHelicopterEntersMap', {
                    location: pos.string
                }), 'heli', Constants.COLOR_PATROL_HELICOPTER_ENTERS_MAP);
            }
            else {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.patrolHelicopterDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'patrolHelicopterLocatedAt', {
                    location: pos.string
                }), 'heli', Constants.COLOR_PATROL_HELICOPTER_LOCATED_AT);
            }
        ...
        for (let marker of leftMarkers) {
            ...
            if (Map.isOutsideGridSystem(marker.x, marker.y, mapSize)) {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.patrolHelicopterLeftSetting, this.client.intlGet(this.rustplus.guildId, 'patrolHelicopterLeftMap', {
                    location: marker.location.string
                }), 'heli', Constants.COLOR_PATROL_HELICOPTER_LEFT_MAP);
                this.timeSincePatrolHelicopterWasOnMap = new Date();
            }
            else {
                this.rustplus.sendEvent(this.rustplus.notificationSettings.patrolHelicopterDestroyedSetting, this.client.intlGet(this.rustplus.guildId, 'patrolHelicopterTakenDown', {
                    location: marker.location.string
                }), 'heli', Constants.COLOR_PATROL_HELICOPTER_TAKEN_DOWN);
    ...
    updateTravelingVendors(mapMarkers) {
        ...
        for (let marker of newMarkers) {
            ...
            this.rustplus.sendEvent(this.rustplus.notificationSettings.travelingVendorDetectedSetting, this.client.intlGet(this.rustplus.guildId, 'travelingVendorSpawnedAt', { location: pos.string }), 'vendor', Constants.COLOR_TRAVELING_VENDOR_LOCATED_AT);
            this.travelingVendors.push(marker);
        }
        /* TravelingVendor markers that have left. */
        for (let marker of leftMarkers) {
            this.rustplus.sendEvent(this.rustplus.notificationSettings.travelingVendorLeftSetting, this.client.intlGet(this.rustplus.guildId, 'travelingVendorLeftMap', { location: marker.location.string }), 'vendor', Constants.COLOR_TRAVELING_VENDOR_LEFT_MAP);
        ...
        for (let marker of remainingMarkers) {
            ...
            if (!this.rustplus.isFirstPoll && !travelingVendor.isHalted) {
                if (marker.x === travelingVendor.x && marker.y === travelingVendor.y) {
                    ...
                    this.rustplus.sendEvent(this.rustplus.notificationSettings.travelingVendorHaltedSetting, this.client.intlGet(this.rustplus.guildId, 'travelingVendorHaltedAt', { location: pos.string }), 'vendor', Constants.COLOR_TRAVELING_VENDOR_HALTED);
                }
            }
            /* If TravelingVendor is moving again */
                ...
                if (marker.x !== travelingVendor.x || marker.y !== travelingVendor.y) {
                    ...
                    this.rustplus.sendEvent(this.rustplus.notificationSettings.travelingVendorHaltedSetting, this.client.intlGet(this.rustplus.guildId, 'travelingVendorResumedAt', { location: pos.string }), 'vendor', Constants.COLOR_TRAVELING_VENDOR_MOVING);
                }
            }
    ...
    notifyCargoShipEgress(args) {
        ...
        this.rustplus.sendEvent(this.rustplus.notificationSettings.cargoShipEgressSetting, this.client.intlGet(this.rustplus.guildId, 'cargoShipEntersEgressStage', {
            location: marker.location.string
        }), 'cargo', Constants.COLOR_CARGO_SHIP_ENTERS_EGRESS_STAGE);
    ...
    notifyCrateSmallOilRigOpen(args) {
        ...
        this.rustplus.sendEvent(this.rustplus.notificationSettings.lockedCrateOilRigUnlockedSetting, this.client.intlGet(this.rustplus.guildId, 'lockedCrateSmallOilRigUnlocked', {
            location: oilRigLocation
        }), 'small', Constants.COLOR_LOCKED_CRATE_SMALL_OILRIG_UNLOCKED, this.rustplus.isFirstPoll, 'locked_crate_small_oil_rig_logo.png');
        this.crateSmallOilRigTimer.stop();
        this.crateSmallOilRigTimer = null;
        this.crateSmallOilRigLocation = null;
    }
    notifyCrateLargeOilRigOpen(args) {
        let oilRigLocation = args[0];
        this.rustplus.sendEvent(this.rustplus.notificationSettings.lockedCrateOilRigUnlockedSetting, this.client.intlGet(this.rustplus.guildId, 'lockedCrateLargeOilRigUnlocked', {
            location: oilRigLocation
        }), 'large', Constants.COLOR_LOCKED_CRATE_LARGE_OILRIG_UNLOCKED, this.rustplus.isFirstPoll, 'locked_crate_large_oil_rig_logo.png');
        this.crateLargeOilRigTimer.stop();
        this.crateLargeOilRigTimer = null;
        this.crateLargeOilRigLocation = null;
    }
