...
class RustPlus extends RustPlusLib {
    ...
    loadRustPlusEvents() {
        const eventFiles = Fs.readdirSync(Path.join(__dirname, '..', 'rustplusEvents')).filter(file => file.endsWith('.js'));
    ...
    updateLeaderRustPlusLiteInstance() {
        ...
        if (leader === this.playerId)
            return;
        if (!(leader in instance.serverListLite[this.serverId]))
            return;
        const serverLite = instance.serverListLite[this.serverId][leader];
        this.leaderRustPlusInstance = new RustPlusLite(this.guildId, this.logger, this, serverLite.serverIp, serverLite.appPort, serverLite.steamId, serverLite.playerToken);
    ...
    updateEvents(event, message) {
        ...
        if (![commandCargoEn, commandHeliEn, commandSmallEn, commandLargeEn, commandChinookEn].includes(event))
            return;
    ...
    replenishTokens() {
        ...
        if (this.tokens > TOKENS_LIMIT)
            this.tokens = TOKENS_LIMIT;
    }
    ...
    async isResponseValid(response) {
        if (response === undefined) {
            this.log(Client.client.intlGet(null, 'errorCap'), Client.client.intlGet(null, 'responseIsUndefined'), 'error');
            return false;
        }
            ...
            default:
                {
                    ...
                    for (const player of teamInfo.teamInfo.members) {
                        if (player.steamId.toString() === callerSteamId) {
                            const direction = Map.getAngleBetweenPoints(player.x, player.y, this.markers[command].x, this.markers[command].y);
                            const distance = Math.floor(Map.getDistance(player.x, player.y, this.markers[command].x, this.markers[command].y));
            ...
            case commandSearch:
                {
                    ...
                    for (const vendingMachine of this.mapMarkers.vendingMachines) {
                        if (!vendingMachine.hasOwnProperty('sellOrders'))
                            continue;
                        for (const order of vendingMachine.sellOrders) {
                            ...
                            const orderItemId = (Object.keys(Client.client.items.items).includes(order.itemId.toString())) ?
                                order.itemId : null;
                            const orderCurrencyId = (Object.keys(Client.client.items.items).includes(order.currencyId.toString())) ?
    ...
    getCommandSteamId(command, callerSteamId, callerName) {
        ...
        if (command.toLowerCase() === `${commandSteamid}` || command.toLowerCase() === `${commandSteamidEn}`) {
            if (callerSteamId === null || callerName === null)
                return null;
            ...
            case commandAdd:
                {
                    ...
                    this.timers[id] = {
                        timer: new Timer.timer(() => {
                            this.sendInGameMessage(Client.client.intlGet(this.guildId, 'timer', { message: message }), 'TIMER');
