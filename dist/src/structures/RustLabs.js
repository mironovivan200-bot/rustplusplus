...
class RustLabs {
    ...
    getDurabilityDetailsByName(name, group = null, which = null, orderedBy = null) {
        if (typeof (name) !== 'string')
            return null;
        if (group !== null && !this.durabilityGroups.includes(group))
            return null;
        if (which !== null && !this.durabilityWhich.includes(which))
            return null;
        if (orderedBy !== null && !this.orderedBy.includes(orderedBy))
            return null;
        let type = null;
        let foundName = null;
        if (!foundName) {
        ...
        for (const item of this.durabilityData[type][foundName]) {
            if (group !== null && item.group !== group)
                continue;
            if (which !== null && item.which !== which)
                continue;
            content.push(item);
        }
    ...
    getDurabilityDetailsById(id, group = null, which = null, orderedBy = null) {
        if (typeof (id) !== 'string')
            return null;
        if (!this.hasDurabilityDetails(id))
            return null;
        if (group !== null && !this.durabilityGroups.includes(group))
            return null;
        if (which !== null && !this.durabilityWhich.includes(which))
