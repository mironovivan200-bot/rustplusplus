...
async function checkBotLeaveVoice(client, oldState, newState) {
    ...
    if (!client.voiceLeaveTimeouts.hasOwnProperty(guildId))
        client.voiceLeaveTimeouts[guildId] = null;
    /* No channel involved. */
    ...
    if (condition && channel && channel.members.size === 1) {
        client.voiceLeaveTimeouts[guildId] = setTimeout(botLeaveVoiceTimeout.bind(null, guildId), Constants.BOT_LEAVE_VOICE_CHAT_TIMEOUT_MS);
    }
}
