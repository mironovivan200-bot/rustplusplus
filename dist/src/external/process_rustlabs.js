...
const RUSTLABS_ITEM_CRAFT_INGREDIENTS_REGEX = /<a\shref.*?img\/.*?\/(.*?)\.png.*?alt="(.*?)".*?text-in-icon">(.*?)<\/span><\/a>/gm;
const RUSTLABS_ITEM_CRAFT_TIME_REGEX = /^\s*<td\sdata-value="(.*?)">(.*?)<\/td>/gm;
const RUSTLABS_ITEM_RESEARCH_AREA_REGEX = /data-name="blueprint"\sclass="tab-page(\n|.)*?<table\sclass(\n|.)*?<\/table>/gm;
const RUSTLABS_ITEM_RESEARCH_ROW_REGEX = /<td\sclass="item-cell">(\n|.)*?<\/tr>/gm;
const RUSTLABS_ITEM_RESEARCH_TYPE_REGEX = /<td\sclass="item-cell">(\n|.)*?<img\sclass=""\ssrc="(\n|.)*?img\/(\n|.)*?\/(.*?)\.png/gm;
...
const RUSTLABS_ITEM_DURABILITY_QUANTITY_GUNS_REGEX = /<img\ssrc=".*?\/img\/.*?\/(.*?)\.png"\salt="(.*?)"\sclass="icon-in-text">(.*?)</gm;
...
const RUSTLABS_ITEM_SMELTING_AREA_REGEX1 = /data-name="smelting"\sclass="tab-page(\n|.)*?<table\sclass(\n|.)*?<thead><tr>\n<th>Process(\n|.)*?<\/table>/gm;
const RUSTLABS_ITEM_SMELTING_AREA_REGEX2 = /<tbody>(\n|.)*?<\/tbody>/gm;
const RUSTLABS_ITEM_SMELTING_AREA_REGEX3 = /<tr>(\n|.)*?<\/tr>/gm;
const RUSTLABS_ITEM_SMELTING_REGEX1 = /<a\shref="\/item\/(\n|.)*?img\/.*?\/(.*?)\.png"\salt="(.*?)"(\n|.)*?<\/a><a\shref="\/item\/(\n|.)*?img\/.*?\/wood\.png"(\n|.)*?text-in-icon">(.*?)<\/span(\n|.)*?<a\shref="\/item\/(\n|.)*?img\/.*?\/(.*?)\.png"\salt="(.*?)"(\n|.)*?text-in-icon">(.*?)<(\n|.)*?<td>(.*?sec|.*?min)</gm;
...
const RUSTLABS_ITEM_DECAY_REGEX1 = /<td>Decay<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>HP<\/td>(\n|.){1,3}?<td>(.*?)<\/td>/gm;
const RUSTLABS_ITEM_DECAY_REGEX2 = /<td>Decay\stime\soutside<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>Decay\stime\sinside<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>HP<\/td>(\n|.){1,3}?<td>(.*?)<\/td>/gm;
const RUSTLABS_ITEM_DECAY_REGEX3 = /<td>Decay\stime\soutside<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>Decay\stime\sunderwater<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>HP<\/td>(\n|.){1,3}?<td>(.*?)<\/td>/gm;
const RUSTLABS_ITEM_DECAY_REGEX4 = /<td>Decay\stime\soutside<\/td>(\n|.){1,3}?<td>(.*?)<\/td>(\n|.)*?<td>HP<\/td>(\n|.){1,3}?<td>(.*?)<\/td>/gm;
const RUSTLABS_ITEM_UPKEEP_AREA_REGEX = /<td>Upkeep<\/td>(\n|.)*?<\/tr>/gm;
const RUSTLABS_ITEM_UPKEEP_REGEX = /img\ssrc=".*?\/img\/.*?\/(.*?)\.png"\salt="(.*?)"(\n|.)*?class="icon-in-text">(.*?)</gm;
...
async function processAllBuildingBlocks() {
    ...
    if (response.status !== 200)
        exit(RUSTLABS_ALL_BUILDING_BLOCKS_URL);
...
function processItemDurability(rustlabsName, shortname, name, data, type = 'items') {
    ...
    for (const match of matches) {
        ...
        if (quantityTypeShortname !== null && quantityTypeName !== null) {
            quantityTypeId = Object.keys(ITEMS).find(e => ITEMS[e].shortname === quantityTypeShortname && ITEMS[e].name === quantityTypeName);
            if (!quantityTypeId)
                exit();
        }
        /* Time */
...
function processItemUpkeep(rustlabsName, shortname, name, data, type = 'items') {
    ...
    for (const match of matches) {
        ...
        const upkeepItemId = Object.keys(ITEMS).find(e => ITEMS[e].shortname === upkeepItemShortname && ITEMS[e].name === upkeepItemName);
        if (!upkeepItemId)
            return;
        content.push({
            id: upkeepItemId,
