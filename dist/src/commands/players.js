...
module.exports = {
    ...
    getData(client, guildId) {
        return new Builder.SlashCommandBuilder()
            ...
            .setRequired(true)
            .addChoices({ name: client.intlGet(guildId, 'online'), value: '0' }, { name: client.intlGet(guildId, 'offline'), value: '1' }, { name: client.intlGet(guildId, 'any'), value: '2' }))
            .addStringOption(option => option
            .setName('name')
...
async function displaySpecificUser(client, interaction, battlemetricsId, playerId) {
    ...
    if (nameChangeHistoryName === '')
        nameChangeHistoryName = client.intlGet(guildId, 'empty');
    if (nameChangeHistoryTime === '')
        nameChangeHistoryTime = client.intlGet(guildId, 'empty');
    if (connectionString === '')
        connectionString = client.intlGet(guildId, 'empty');
    if (connectionTime === '')
        connectionTime = client.intlGet(guildId, 'empty');
    const fields = [
        {
    ...
    client.log(client.intlGet(interaction.guildId, 'infoCap'), client.intlGet(interaction.guildId, 'displayingOnlinePlayers'));
}
