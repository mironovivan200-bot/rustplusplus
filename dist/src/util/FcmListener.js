...
module.exports = async (client, guild) => {
    ...
    if (Object.keys(credentials).length === 1) {
        client.log(client.intlGet(null, 'warningCap'), client.intlGet(null, 'credentialsNotRegisteredForGuild', { id: guild.id }));
        return;
    }
    if (!hoster) {
        client.log(client.intlGet(null, 'warningCap'), client.intlGet(guild.id, 'credentialsHosterNotSetForGuild', { id: guild.id }));
        return;
    }
    /* Destroy previous instance of fcm listener */
    if (client.fcmListeners[guild.id])
        client.fcmListeners[guild.id].destroy();
                        ...
                        case 'entity':
                            {
                                switch (body.entityName) {
                                    ...
                                    default:
                                        {
                                            client.log('FCM Host', `GuildID: ${guild.id}, SteamID: ${hoster}, ` +
...
async function alarmAlarm(client, guild, title, message, body) {
    ...
    const server = instance.serverList[serverId];
    const rustplus = client.rustplusInstances[guild.id];
    if (!server || (server && !server.alarms[entityId]))
        return;
...
async function teamLogin(client, guild, title, message, body) {
    const instance = client.getInstance(guild.id);
    const content = {
        embeds: [DiscordEmbeds.getTeamLoginEmbed(guild.id, body, await Scrape.scrapeSteamProfilePicture(client, body.targetId))]
    };
    const rustplus = client.rustplusInstances[guild.id];
    const serverId = `${body.ip}-${body.port}`;
    if (!rustplus || (rustplus && (serverId !== rustplus.serverId))) {
        await DiscordMessages.sendMessage(guild.id, content, null, instance.channelId.activity);
        client.log(client.intlGet(null, 'infoCap'), client.intlGet(null, 'playerJustConnectedTo', {
