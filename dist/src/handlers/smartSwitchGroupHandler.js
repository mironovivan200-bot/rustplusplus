...
module.exports = {
    ...
    smartSwitchGroupCommandHandler: async function (rustplus, client, command) {
        ...
        const groupId = Object.keys(switchGroups).find(e => command === `${prefix}${switchGroups[e].command}` ||
        ...
        if (command.startsWith(`${groupCommand} ${onEn}`) || command.startsWith(`${groupCommand} ${onLang}`)) {
            ...
            const statusMessage = switchStatus.map(status => `${status.name}: ${status.reachable ? (status.active ? onCap : offCap) : notFoundCap}`).join(', ');
